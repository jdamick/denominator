...
import static denominator.ultradns.Transformers.toRDataFunction;
import static denominator.ultradns.Transformers.toValueFromTypeFunction;
...
class GroupByRecordNameAndTypeIterator implements Iterator<ResourceRecordSet<?>> {
    ...
    public ResourceRecordSet<?> next() {
        ...
        Builder<Map<String, Object>> builder = ResourceRecordSet.builder()
                                                                .name(record.getName())
                                                                .type(toValueFromTypeFunction().apply(record.getType()))
                                                                .ttl(record.getTTL());

        builder.add(toRDataFunction().apply(record));
